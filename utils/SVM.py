...
def treinar_svm(X_train, y_train, kernel='rbf', C=1.0, gamma='scale'):
    """
    Treina o classificador SVM com os parâmetros especificados.
    """
...
def plot_classificacao_resultados(X_test, y_test, y_pred, label_encoder):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name=f"Teste Correto ({correct_count})",  # Adiciona contagem [[1]][[3]]
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name=f"Teste Incorreto ({incorrect_count})",  # Adiciona contagem [[4]]
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            xanchor="right", 
            x=1,
            bgcolor="rgba(255,255,255,0.8)"  # Fundo branco para legibilidade [[2]]
        ),
        hoverlabel=dict(bgcolor="white", font_size=12)
    )
    fig.show()

def plot_train_test_mix_manual(X_test, label_encoder):
    """
    Plota um gráfico interativo apenas com os pontos de teste em cinza
    """
    fig = make_subplots(rows=1, cols=1,
...
def treinar_svm_com_validacao_cruzada_treino(X_train, y_train, kernel='rbf', C=1.0, gamma='scale', cv=5):
    """
    Treina o classificador SVM com validação cruzada SOMENTE no conjunto de treino,
    exibe gráfico com resultados e retorna o modelo final treinado com todo o treino.
    ...
    Retorna:
    - modelo SVM treinado com todos os dados de treino
    """
    print(f"\n🔁 Avaliando SVM com validação cruzada no treino ({cv} folds)...")
    svm_model = SVC(kernel=kernel, C=C, gamma=gamma, random_state=42)
    ...
    print(f"📉 Desvio padrão da validação cruzada: {scores.std() * 100:.2f}%")

    # Treina modelo final com todo o conjunto de treino
    svm_model.fit(X_train, y_train)

    # Visualização com Plotly
    df_plot = pd.DataFrame({'Fold': list(range(1, cv + 1)), 'Acurácia': scores})
    fig = px.bar(df_plot, x='Fold', y='Acurácia',
                 title='Acurácia por Fold na Validação Cruzada (SVM)',
...
def main_svm():
    ...
    # Validação cruzada com visualização
    modelo_svm = treinar_svm_com_validacao_cruzada_treino(
        X_train, y_train, 
        kernel='rbf',  # Alterado para linear conforme exemplo
        C=1.0, 
        cv=5  # 5 folds
    )
    
    # Avaliação final no conjunto de teste
    y_pred = avaliar_modelo(modelo_svm, X_test, y_test, le)

    # Plotagem dos dados de teste
    plot_train_test_mix_manual(X_test, le)
